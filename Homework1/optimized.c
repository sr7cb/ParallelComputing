...
int main(int argc, char *argv[])
{
    ...
    for (i=1; i<=natom; ++i)
    {
		long idiff = i -1; /*moved subtraction from vec2 to variables*/
        for (j=1; j<i; ++j) /*moved j < i into second loop*/
        {
			long jdiff = j-1;
				/*Optimized out the power function and the subtraction calculation */ 
                vec2 = ((coords[0][idiff]-coords[0][jdiff]) * (coords[0][idiff]-coords[0][jdiff]))
                       + ((coords[1][idiff]-coords[1][jdiff]) * (coords[1][idiff]-coords[1][jdiff]))
                       + ((coords[2][idiff]-coords[2][jdiff]) * (coords[2][idiff]-coords[2][jdiff]));
                /* X^2 + Y^2 + Z^2 */
                /* Check if vec2 is below the cut off squared */
                if ( vec2 <= (cut*cut) ) 
                {
	
					rij = sqrt(vec2); /* if condtion met then do sqrt function*/
					double one_over_r = 1.0/rij; /* created a variable to make division multiplication*/
                    /* Increment the counter of pairs below cutoff */
                    ++cut_count;
                    current_e = (exp((rij*q[i-1])+(rij*q[j-1]))) * one_over_r; /* optimized the exponential to 1 call and made division into multiplication */
                    total_e = total_e + current_e - c; /*moved constant calculation outside of loop*/
                }
        } /* for j=1 j<=natom */
    } /* for i=1 i<=natom */
